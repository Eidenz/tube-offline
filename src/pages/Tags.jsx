...
const Tags = () => {
  ...
  const handleTagClick = (tag) => {
    navigate(`/search?q=${encodeURIComponent(tag.name)}&type=tag`);
  };
  ...
  return (
    ...
    >
      ...
      {/* Full-width tags cloud without the sidebar */}
      <motion.div 
        className="bg-secondary/20 rounded-xl p-6 min-h-[400px]"
            ...
            <div className="flex flex-wrap gap-3">
              {visibleTags.map((tag) => {
                ...
                return (
                  <motion.button
                    key={tag.id}
                    className={`px-4 py-2 rounded-full flex items-center gap-1 transition-colors ${colorSet.bg} ${colorSet.text} ${colorSet.hover}`}
      ...
      {/* Note: We've removed the tag details panel completely */}
    </motion.div>
  );
};
